let request = require('request'),
	...
	isLoginForm = ($) => {
		let listOfInputs = [], loginFormDoc = $('input'), loginInPossibleValues = ['signin', 'login'], passwordFieldCount = 0, emailOrTextCount = 0, loginButton = 0;
	  	$('form').find('input').each(function(index, el){
    		...
    		if(type === 'text' || type === 'email') {
    			...
    			var name = $(el).attr('name') && $(el).attr('name').toLowerCase().split(' ').join(''), 
    				val = $(el).attr('value') && $(el).attr('value').toLowerCase().split(' ').join('');
    			if(loginInPossibleValues.indexOf(name) != -1 || loginInPossibleValues.indexOf(val) != -1) {
					//If the text input type is submit and containing ['signin', 'login'] as a name or value then increse the loginButton count
					loginButton ++;
    			}
    		}
    		listOfInputs.push({type: type, name: $(el).attr('name'), val: $(el).attr('value')});
	    });
    	...
    	$('a').each(function(index, el) {
    		...
    		if(href) {
	    		if(href.match(externalLinkRegEx)) {
	    			...
	    			links.push({link: selectedUri.split('//')[0] + '//' + host + href, type: 'internal'});
	    		}
	    	}
	    });
    	return links;
	...
	checkResponse = (uri, type, host) => {
		return new Promise((resolve, reject) => {
			sendRequest(uri, 5).then((resp) => {
				let linkHost = resp.response.request.host;
				console.log(host + '  --------->  ' + linkHost)
				if(resp.response.request && resp.response.request.host && resp.response.request.host === host) {
	...
	sendRequest = (uri, redirectCount) => {
		...
		if(!uri.match(validProtocol)) {
			if(uri.match(withoutProtocol)) {
				...
				if(hostName.match(validHostnameRegex) || hostName.match(validIpAddressRegex)) {
					uriToCheck = 'http://' + uri;
				}
			} 
		}
	...
	fetchLinksStatus = (links, host) => {
		return new Promise((resolve, reject) => {
		    ...
		    Promise.all(promises).then((values) => {
		    	for(let p in links) {
		    		linksVerified[values[p].type].push({link: values[p].link, accessible: values[p].accessible});
		    	}
		    	let linksWithStatus = [
		    		{
		    			type: 'External',
...
exports.isLoginForm = isLoginForm;
exports.checkResponse = checkResponse;
exports.getHtmlVersion = getHtmlVersion;
exports.getAllLinks = getAllLinks;
exports.sendRequest = sendRequest;	
exports.getAllHeadingsCount = getAllHeadingsCount;	
exports.fetchLinksStatus = fetchLinksStatus;	
